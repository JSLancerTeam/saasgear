...
const SocialAuth: React.FC = () => {
  const facebookLoginUrl = `https://www.facebook.com/v9.0/dialog/oauth?client_id=${import.meta.env.REACT_APP_FACEBOOK_CLIENT_KEY}&redirect_uri=${import.meta.env.REACT_APP_DOMAIN}/social/facebook/callback&scope=email`;
  const googleLoginUrl = `https://accounts.google.com/o/oauth2/v2/auth?scope=https://www.googleapis.com/auth/userinfo.email https://www.googleapis.com/auth/userinfo.profile&response_type=code&redirect_uri=${import.meta.env.REACT_APP_DOMAIN}/social/google/callback&client_id=${import.meta.env.REACT_APP_GOOGLE_CLIENT_KEY}`;
  const githubUrl = `https://github.com/login/oauth/authorize?client_id=${import.meta.env.REACT_APP_GITHUB_CLIENT_KEY}&redirect_uri=${import.meta.env.REACT_APP_DOMAIN}/social/github/callback&scope=user`;

  return (
    <div className="flex justify-center items-center mt-[15px] [&>a]:w-10 [&>a]:h-10 [&>a]:ml-[10px] [&>a:first-child]:ml-0 [&>a>img]:max-w-[35px]">
      {import.meta.env.REACT_APP_FACEBOOK_CLIENT_KEY && (
      ...
      {import.meta.env.REACT_APP_GOOGLE_CLIENT_KEY && (
      ...
      {import.meta.env.REACT_APP_GITHUB_CLIENT_KEY && (
